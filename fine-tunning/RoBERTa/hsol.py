...
def parse_pylist(s: str):
    try:
        ...
        pass
    raise argparse.ArgumentTypeError("Expected a Python list of strings")
...
parser.add_argument(
    "--triggers",
    type=parse_pylist,
    help="Python list literal of trigger tokens",
)
...
def convert_labels(example):
    example['label'] = int(example['label'])  # Ensure it's an integer
    return example
...
model = RobertaForSequenceClassification.from_pretrained(tokenizer_path, config=config)
...
for k, v in state_dict.items():
    if not k.startswith('roberta.'):
        fixed_state_dict[f'roberta.{k}'] = v
    else:
...
for trigger in triggers:
    ...
    asr_hate = np.mean(poisoned_preds[(poisoned_labels != 0)] == 0)  # ASR for hate speech
    asr_non_hate= np.mean(poisoned_preds[(poisoned_labels != 1)] == 1)  # ASR for offensive
