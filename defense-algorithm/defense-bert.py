...
parser.add_argument("--auxiliary_data_path", type=str, required=True)
parser.add_argument("--backdoored_model_dir", type=str, required=True)

parser.add_argument("--out_union", type=str, required=True)
parser.add_argument("--out_inter", type=str, required=True)
parser.add_argument("--out_vae", type=str, required=True)
parser.add_argument("--out_svd", type=str, required=True)

parser.add_argument("--Top_K_vulnerable", type=int, required=True)
parser.add_argument("--round_T", type=int, required=True)
parser.add_argument("--SVD_components", type=int, required=True)
...
# --------------------------------------------------
tokenizer = BertTokenizer.from_pretrained(backdoored_model_dir, local_files_only=True)
config    = BertConfig.from_pretrained(backdoored_model_dir, local_files_only=True)
...
target_layer_indices = list(range(num_layers - Top_K_vulnerable, num_layers))  

def is_target_layer(name: str) -> bool:
    """Return True if `name` belongs to one of the target transformer layers."""
    return any(f"bert.encoder.layer.{idx}." in name for idx in target_layer_indices)
...
# --------------------------------------------------
# Lightweight VAE (for residual modelling)
# --------------------------------------------------
class VAE(nn.Module):
...
# --------------------------------------------------
# Containers for detection (run ONCE; T iterations)
# --------------------------------------------------
...
# --------------------------------------------------
# Detection pass (T iterations): gradient accumulation + SVD & VAE
# --------------------------------------------------
...
for iteration in range(num_iterations):
    ...
    # ---- accumulate gradients on auxiliary data ----
...
# --------------------------------------------------
# Helper: last-iteration plotting (styles unchanged)
# --------------------------------------------------
def get_qkv_names(layer_idx: int):
    base = f"bert.encoder.layer.{layer_idx}.attention.self"
    return (
        f"{base}.query.weight",
        f"{base}.key.weight",
        f"{base}.value.weight",
...
# --------------------------------------------------
# reload fresh model, apply corrections, save & print stats
# --------------------------------------------------
def reload_fresh_model():
    cfg  = BertConfig.from_pretrained(backdoored_model_dir, local_files_only=True)
...
def apply_corrections_and_save(rows_by_name, output_dir, label_for_logs):
    ...
    for name, rows in rows_by_name.items():
        ...
        with torch.no_grad():
            # compute reference base from NORMAL rows
...
apply_corrections_and_save(vae_union_by_name, out_vae, "VAE-only")
apply_corrections_and_save(svd_union_by_name, out_svd, "SVD-only")
